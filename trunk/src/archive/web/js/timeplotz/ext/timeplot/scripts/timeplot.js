 ...
 */
Timeplot.create = function(elmt, plotInfos, trendlines) {
    return new Timeplot._Impl(elmt, plotInfos, trendlines);
};
...
Timeplot.createPlotInfo = function(params) {
    return {   
		...
		hideValueFlag:     ("hideValueFlag" in params) ? params.hideValueFlag : false,
 ...
 * @constructor 
 */
Timeplot._Impl = function(elmt, plotInfos, trendlines) {
...
Timeplot._Impl.prototype = {
        ...
        if (styles) {
            for (style in styles) {
                if (style == "left") {
                    ...
                    if (styles[style] < 0 || isNaN(styles[style])) styles[style] = 0;
                    styles[style] += "px";
    ...
    _initialize: function() {
        ...
        if (this._isBrowserSupported(canvas)) {
            ...
            this._plots = [];
            if (this._plotInfos) {
                for (var i = 0, j = this._plotInfos.length; i < j; i++) {
                    var plot = new Timeplot.Plot(this, this._plotInfos[i], this._trendlines);
                    var dataSource = plot.getDataSource();
                    if (dataSource) {
