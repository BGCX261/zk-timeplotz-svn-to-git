 ...
 */
//ZK add
function getFloatNum(arg1){
	try {
		return arg1.toString().split(".")[1].length
	} catch(e) {
		return 0;
	}	
}
function accAdd(arg1, arg2){	
	var m = Math.pow(10, Math.max(getFloatNum(arg1), getFloatNum(arg2))); 
...
Timeplot.DefaultValueGeometry.prototype = {
    ...
    _calculateGrid: function() {
        ...
        var inc = unit;
		var $node = zk(this._timeplot._containerDiv);
        while (true) {
            var dy = this.toScreen(this._minValue + inc);

	        while (dy < this._gridSpacing && $node.isRealVisible()) {
...
Timeplot.DefaultTimeGeometry = function(params) {
	...
	this._formats = ("formats" in params) ? params.formats : null;
	// used by plot.js initialize#mouseMoveHandler
	this._displayMilli = ("displayMilli" in params) ? params.displayMilli : "FALSE";
	this._measureDensityOfMillisecond = ("measureDensityOfMillisecond" in params) ? parseInt(params.measureDensityOfMillisecond) : 50;
...
Timeplot.DefaultTimeGeometry.prototype = {
    ...
    toScreen: function(time) {
    	if (this._canvas && this._latestDate && this._earliestDate) {
    ...
    fromScreen: function(x) {
    	if (this._canvas && this._earliestDate) {
    ...
    */
    ...
    _calculateGrid: function() {
        ...
        if (p > time.gregorianUnitLengths[time.MILLENNIUM]) {
	        ...
	        for (unit = time.MILLENNIUM; unit >= 0; unit--) {
        ...
        var t = u.cloneValue(this._earliestDate),
			...
			opts; // added for millisecond increment
        ...
        do {
	        ...
	        switch (unit) {
	        	case time.MILLISECOND:
	        		var format = formats['org.zkforge.timeplot.geometry.FORMAT_MILLISECOND'],//ZK add
	        			l;
	        		opts = {};
	        		opts.incrementValue = this._measureDensityOfMillisecond;
	        		l = format ? zk.fmt.Date.formatDate(t,format):
	        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.toLocaleTimeString();
	        	  break;
	        	case time.SECOND:
                  var format = formats['org.zkforge.timeplot.geometry.FORMAT_SECOND'],//ZK add
				  	  l = format ? zk.fmt.Date.formatDate(t,format):
		        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.toLocaleTimeString();
	        	  break;
				case time.QUATERMINUTE:
	        	  var format = formats['org.zkforge.timeplot.geometry.FORMAT_QUATERMINUTE'],//ZK add
				  	  m = t.getMinutes(),
				  	  s = t.getSeconds(),
                  	  l = format ? zk.fmt.Date.formatDate(t,format):
  	        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.getHours() + ":" + ((m < 10) ? "0" : "") + m + ":" + ((s < 10) ? "0" : "") + s;
                  break;
	        	case time.MINUTE:
	        	  var format = formats['org.zkforge.timeplot.geometry.FORMAT_MINUTE'],//ZK add
				      m = t.getMinutes(),
				  	  l = format ? zk.fmt.Date.formatDate(t,format):
		        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.getHours() + ":" + ((m < 10) ? "0" : "") + m;
                  break;
				case time.QUATERHOUR:
	        	  var format = formats['org.zkforge.timeplot.geometry.FORMAT_QUATERHOUR'],//ZK add
				  	  m = t.getMinutes(),
				  	  l = format ? zk.fmt.Date.formatDate(t,format):
		        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.getHours() + ":" + ((m < 10) ? "0" : "") + m;
                  break;
                case time.HOUR:
                  var format = formats['org.zkforge.timeplot.geometry.FORMAT_HOUR'],//ZK add
				  	  l = format ? zk.fmt.Date.formatDate(t,format):
		        			this._displayMilli == 'TRUE'? this._getMilliLabel(t) : t.getHours() + ":00";
                  break;
	        	case time.DAY:
	        	case time.WEEK:
                case time.MONTH:
				  var format = formats['org.zkforge.timeplot.geometry.FORMAT_DAY'],//ZK add
				  	  l = format ? zk.fmt.Date.formatDate(t,format): t.toLocaleString();
                  break;  
                case time.YEAR:
                case time.DECADE:
                case time.CENTURY:
                case time.MILLENNIUM:
	        	  var format = formats['org.zkforge.timeplot.geometry.FORMAT_YEAR'],//ZK add
				  	  l = format ? zk.fmt.Date.formatDate(t,format): t.getUTCFullYear();
	        	  break;
	        }
	        if (x > 0) { 
		        grid.push({ x: x, label: l });
	        }
	        time.incrementByInterval(t, unit, this._timeZone, opts);
        } while (t.getTime() < this._latestDate.getTime());
        return grid;
    },
	
	// create millisecond label
	_getMilliLabel: function (t) {
		var h = t.getHours(),
			m = t.getMinutes(),
			s = t.getSeconds(),
    ...
    _updateMappedValues: function() {
    	...
    	} else if (this._max && this._min && !this._latestDate && !this._earliestDate) {
	        this._period = this._max.getTime() - this._min.getTime();
	        this._mappedPeriod = this._map.direct(this._period);
			this._latestDate = new Date(this._max);
			this._earliestDate = new Date(this._min);
    	} else {
    		this._period = 0;
...
Timeplot.MagnifyingTimeGeometry.prototype.setMagnifyingParams = function(c,a,b) {
    ...
    var ct = this.fromScreen(c) - this._earliestDate ? this._earliestDate.getTime() : 0;
    if (ct - b < 0) b = ct;
    if (ct + b > d) b = d - ct;
