 ...
 * @constructor
 */
Timeplot.DefaultEventSource.NumericEvent = function(time, values, customValues, format) {
    this._id = "e" + Math.round(Math.random() * 1000000);
    this._time = time;
    this._values = values;
	this._customValues = customValues; //ZK add
	this._format = format; //ZK add
};
...
Timeplot.DefaultEventSource.NumericEvent.prototype = {
	...
	getCustomValues:      function() { return this._customValues; }, //ZK add
	getFormat:      function() { return this._format; }, //ZK add
...
Timeplot.DataSource.prototype = {
    ...
    getValue: function(t) {
    	if (this._data) {
	    	for (var i = 0, j = this._data.times.length; i < j; i++) {
	    		if (this._data.times[i] >= t) {
